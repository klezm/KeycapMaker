...
test("TTF / OTF direct links resolve by verifying actual font signature", async (t) => {
...
test("TTF / OTF direct links reject non-font responses", async (t) => {
  const originalFetch = globalThis.fetch;
  t.after(() => {
    globalThis.fetch = originalFetch;
  });
...
test("@font-face CSS resolves TTF / OTF src", async (t) => {
  const originalFetch = globalThis.fetch;
  t.after(() => {
    globalThis.fetch = originalFetch;
  });
